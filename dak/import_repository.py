...
def entry_arch_exists(entry, packages):
    # print(packages[entry['Package']])
    for binary_object in packages[entry['Package']]:
        # print(str(binary_object.architecture))
        # print(entry['Architecture'])
        if entry['Architecture'] == str(binary_object.architecture):
            return True
    return False
...
def import_sources(base, sources, transaction, target_suite, component, target_sources, extra_sources, extra_sources_comp, max_packages=None):
    ...
    for entry in sources:
        ...
        if entry.get('Extra-Source-Only', 'no') == 'yes':
            ...
            extra_sources_comp[key].add(component)
            continue
...
def import_built_using(base, source, version, transaction, target_suite, component, extra_sources, extra_sources_comp):
    if not daklib.import_repository.source_in_archive(source, version, target_suite.archive):
        print("Importing extra source {0}={1}".format(source, version))
        key = (source, version)
        extra_entry = extra_sources.get(key)

        ## removed architecture argument. who cares
        if extra_entry is None:
            raise Exception("Extra source {0}={1} referenced by {2}={3} ({4}) not found in source suite.".format(source, version, entry['Package'], entry['Version']))
        extra_components = extra_sources_comp[key]

        ## this is just a temporary fix. not sure it is correct.
...
def import_packages(base, packages, transaction, target_suite, component, architecture, target_binaries, extra_sources, extra_sources_comp, max_packages=None):
    ...
    for entry in packages:
        if max_packages is not None and n > max_packages:
            break
        ## added architecture checks. (package, version, architecture) triplet must be unique.
        if not entry_in_packages(entry, target_binaries) or entry_is_newer(entry, target_binaries) or not entry_arch_exists(entry, target_binaries):
...
def main(argv=None):
    ...
    with daklib.archive.ArchiveTransaction() as transaction:
        ...
        ## don't know why this line even exists... comment and leave it anyway
