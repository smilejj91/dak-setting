...
def import_package_to_suite(base, entry, transaction, suite, component):
    ...
    ## this is just a temporary fix. again, not sure what is going to happen if remove sha1 completely. will comeback soon
    if 'SHA1' not in entry:
        hashedfile = daklib.upload.HashedFile(os.path.basename(filename), int(entry['Size']), entry['MD5sum'], '', entry['SHA256'], input_filename=fn)
    else:
